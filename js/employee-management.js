...
const EmployeeManagement = {
  ...
  render(){
    ...
    const active=this.records.filter(x=>(x.status||(x.active===false?'inactive':'active'))==='active').length;
    ...
    host.innerHTML=list.map(x=>{
      ...
      return `<article class="employee-directory-card">
        <div class="employee-directory-main"><img src="${this.esc(photo)}" onerror="this.src='icons/head_logo.webp'" alt="${this.esc(x.name)}"><div><span class="employee-status ${st}">${st==='active'?'সক্রিয়':st==='pending'?'অপেক্ষমাণ':'নিষ্ক্রিয়'}</span><h3>${this.esc(x.name||'নাম দেওয়া হয়নি')}</h3><p>${this.esc(x.designation||cfg.designation||'Team Member')}</p><b>${this.esc(x.employeeId||'ID অপেক্ষমাণ')}</b></div></div>
        <dl><div><dt>বিভাগ</dt><dd>${this.esc(x.department||cfg.department||'—')}</dd></div><div><dt>অফিস ডেস্ক</dt><dd>${this.esc(x.workspaceName||cfg.workspace||'—')}</dd></div><div><dt>শাখা</dt><dd>${this.esc(x.branchName||x.branchZone||'Head Office')}</dd></div><div><dt>ড্রেস কোড</dt><dd>${this.esc(x.uniform||cfg.uniform||'Company Uniform')}</dd></div></dl>
        <div class="employee-directory-actions"><button onclick="EmployeeManagement.showId('${this.esc(x.uid)}')">🪪 ID Card</button><button onclick="EmployeeManagement.openEdit('${this.esc(x.uid)}')">✏️ এডিট</button><button class="danger" onclick="EmployeeManagement.toggleStatus('${this.esc(x.uid)}')">${st==='active'?'ডেস্ক স্থগিত':'সক্রিয় করুন'}</button></div>
      </article>`;
    }).join('');
  },
  nextId(role){
  ...
  openCreate(){
    this.populateControls(); this.editingUid=null; this.photoFile=null;
    ['empName','empUid','empEmail','empPhone','empDesignation','empDepartment','empWorkspace','empEmployeeId','empUniform','empResponsibilities'].forEach(id=>{const e=document.getElementById(id);if(e)e.value=''});
    document.getElementById('empRole').value='support'; document.getElementById('empStatus').value='active';
  ...
  async save(){
    ...
    try{
      ...
      const status=get('empStatus')||'active'; const payload={name,email:get('empEmail'),phone:get('empPhone'),role,designation:get('empDesignation'),department:get('empDepartment'),workspaceName:get('empWorkspace'),employeeId,branchName:get('empBranch')||'Head Office',uniform:get('empUniform'),responsibilities:get('empResponsibilities'),status,active:status==='active',photoURL,updatedAt:FB.serverTimestamp()};
